...
| 탭 | 기능 | 활용 시점 |
|----|------|----------|
| 📋 포트폴리오 | 종목 추가/삭제, 수량 인라인 편집, 실시간 시세 및 원화/달러 평가금액, 📸 증권사 캡쳐 이미지로 수량 자동 업데이트 | 매주 현황 확인 및 수량 업데이트 |
| 📡 AI 시그널 | 보유 종목 뉴스 AI 분석 (Gemini + Finnhub + Marketaux), 상승/하락/중립 카드 UI, 뉴스/애널리스트/액션/AI 의견 4개 bullet | 투자 결정 전 뉴스 흐름 파악 |
...
- **[신규]** 📸 포트폴리오 캡쳐 이미지 업로드로 수량 자동 업데이트
...
- **[신규]** 총 평가금액 USD 메트릭 카드 추가 (현재가 × 수량 직접 합산)
- **[개선]** AI 시그널 카드 — 4번째 bullet에 "AI 의견" 라벨 추가
- **[개선]** Marketaux API 키 지원 추가 (뉴스 품질 향상)
- **[개선]** 앱 시작 시 모든 API 키 자동 로드 (탭 진입 전 세션 초기화)
- **[BUG FIX]** signal_cache UTC/KST 날짜 불일치 버그 수정 (자정 00:00~08:59 KST 구간 캐시 미로드 문제)
