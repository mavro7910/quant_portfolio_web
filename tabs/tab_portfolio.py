...
def render(portfolio: Portfolio):
    ...
    # ── 종목 추가/수정 폼 ─────────────────────────────────
    st.markdown(section_title("종목 관리"), unsafe_allow_html=True)

    col_t, col_s, col_btn1 = st.columns([2.5, 2.5, 1.5])
    with col_t:
        new_ticker = st.text_input("티커", placeholder="AAPL", key="inp_ticker").upper().strip()
    with col_s:
        new_shares = st.number_input("보유 수량", min_value=0.0, max_value=9_999_999.0,
    ...
    with st.expander("📸 증권사 캡쳐로 포트폴리오 업데이트", expanded=False):
        if not has_api_key():
            ...
            uploaded = st.file_uploader("이미지 업로드 (JPG/PNG)", type=["jpg","jpeg","png"],
                                        accept_multiple_files=True, key="portfolio_img_uploader")
            if uploaded:
        ...
        if "img_extracted" in st.session_state:
            ...
            st.markdown(section_title("✏️ 추출 결과 확인 · 수정 후 반영하세요"), unsafe_allow_html=True)
            df_preview = pd.DataFrame([{
                "티커": item.get("ticker",""), "한글명": item.get("name_kr",""),
            ...
            edited = st.data_editor(
                ...
                hide_index=True, width="stretch", num_rows="fixed", key="img_preview_editor",
            )
            ...
            if missing:
                st.markdown(banner(f"⚠️ 이미지에 없는 종목은 수량이 <b>0</b>으로 변경됩니다: {', '.join(missing)}", "warn"), unsafe_allow_html=True)
            ...
            with col_apply:
                if st.button("✅ 포트폴리오에 반영", key="btn_apply_img", width="stretch"):
                    ...
                    if zeroed: msg += f" · {len(zeroed)}개 0으로 초기화 ({', '.join(zeroed)})"
                    st.success(msg)
                    st.rerun()
            with col_cancel:
