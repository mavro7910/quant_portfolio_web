...
<b>🔬 KH 전략 작동 방식 (v3 · Z-score 가중합)</b><br><br>
<b>① 시장 국면 판단 (Bull / Bear)</b><br>
QQQ 현재가가 MA200 위면 <b>강세장</b>, 아래면 <b>약세장</b>.<br><br>
<b>② 팩터 Z-score 계산</b><br>
· <b>모멘텀</b>: 21일(10%) · 63일(20%) · 126일(30%) · 252일(40%) 수익률 Z-score 가중합<br>
· <b>변동성역수</b>: 60일 표준편차 역수 Z-score (낮은 변동성 → 높은 점수)<br>
Z-score는 종목 간 실제 격차를 반영합니다 (rank 방식 대비 AQR 검증 방식).<br><br>
<b>③ 국면별 가중합 → ReLU → 25% 캡</b><br>
· 강세장: 모멘텀_z × 0.7 + 변동성역수_z × 0.3<br>
· 약세장: 모멘텀_z × 0.4 + 변동성역수_z × 0.6<br>
음수 alpha 종목은 0으로 처리(자연 배제) → 정규화 → 단일 종목 최대 25% 캡.<br><br>
...
""", unsafe_allow_html=True)
    ...
    mcap_label = _preset_label.get(mcap_preset, mcap_preset)
