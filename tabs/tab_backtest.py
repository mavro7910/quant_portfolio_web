...
<b>② 팩터 점수 계산</b> — 모두 0~1 rank로 동일 스케일<br>
· <b>모멘텀 점수</b>: 21일(10%) · 63일(20%) · 126일(30%) · 252일(40%) 수익률의 가중 순위<br>
· <b>변동성역수 점수</b>: 60일 변동성이 낮을수록 높은 점수<br><br>
<b>③ 코어 알파 — 가중 기하평균(곱)</b><br>
<code>core = 모멘텀 ^ m_w  ×  변동성역수 ^ v_w</code><br>
두 팩터 <b>모두 좋아야</b> 높은 비중. 하나라도 낮으면 곱이 작아져 강한 패널티.<br><br>
<b>④ 국면별 지수 배합</b><br>
· 강세장: m_w=0.7, v_w=0.3 → 모멘텀 주도<br>
· 약세장: m_w=0.4, v_w=0.6 → 변동성방어 주도<br><br>
<b>⑤ 시총 팩터 — 독립 가산(γ=15% or 0%)</b><br>
<code>최종비중 = core × (1-γ)  +  시총rank × γ</code><br>
시총 가중 ON 시 γ=15%로 소량 참고. OFF 시 γ=0% (순수 팩터 알파).<br><br>
