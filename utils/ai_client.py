...
def analyze_portfolio_signals(
    ...
    for t in tickers:
        ...
        if _needs_reanalysis(t, change_pct, cached_tickers):
            reanalyze_tickers.append(t)
        else:
            keep_tickers.append(t)

    # ── 3단계: 재분석 대상만 애널리스트 데이터 수집 ───────────────
    if progress_callback and reanalyze_tickers:
        progress_callback(0, len(reanalyze_tickers), "애널리스트 데이터 수집 중", None)

    analyst_ctx = (
        fetch_analyst_data(reanalyze_tickers, finnhub_key=finnhub_key)
        if reanalyze_tickers
        else {}
    )

    # ── 4단계: 재분석 대상만 Gemini 배치 분석 ───────────────────
    signal_map: dict[str, dict] = {}

    if reanalyze_tickers:
        if progress_callback:
            progress_callback(1, len(reanalyze_tickers), "AI 분석 중", None)
