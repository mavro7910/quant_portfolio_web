...
with tab1:
    ...
    with col_t:
        new_ticker = st.text_input("티커", placeholder="AAPL", key="inp_ticker",
                                   label_visibility="collapsed").upper().strip()
    with col_s:
        new_shares = st.number_input("수량", min_value=0.000001, max_value=9_999_999.0,
                                     value=1.0, step=0.1, format="%.6f",
                                     key="inp_shares", label_visibility="collapsed")
    with col_btn1:
    ...
    with col_btn2:
        ...
        del_ticker = st.selectbox("삭제할 종목", ["선택..."] + tickers_list,
                                  key="del_select", label_visibility="collapsed")
    ...
    if not holdings:
        ...
        if "prices_data" in st.session_state:
            ...
            if fx_est:
                st.markdown('<div class="warn-banner">⚠️ USD/KRW 환율 조회 실패 -- 추정값(₩1,480) 사용 중</div>',
                            unsafe_allow_html=True)
            ...
            for t, s in holdings.items():
                ...
                val = p * s * fx
                total_krw += val
                rows.append((t, f"{s:.6f}", f"${p:,.2f}", f"₩{val:,.0f}"))

            df_hold = pd.DataFrame(rows, columns=["티커", "보유 수량", "현재가 (USD)", "평가금액 (KRW)"])

            # 요약 지표
            c1, c2, c3 = st.columns(3)
            c1.markdown(f'<div class="metric-card"><div class="label">보유 종목 수</div><div class="value">{len(holdings)}개</div></div>', unsafe_allow_html=True)
            c2.markdown(f'<div class="metric-card"><div class="label">총 평가금액</div><div class="value">₩{total_krw:,.0f}</div></div>', unsafe_allow_html=True)
            c3.markdown(f'<div class="metric-card"><div class="label">USD/KRW</div><div class="value">{fx:,.2f}</div></div>', unsafe_allow_html=True)
            st.write("")
...
with tab2:
    ...
    with col_m:
        use_mcap = st.checkbox("시가총액 가중 사용", value=True,
    ...
    if "buy_result" in st.session_state:
        ...
        if fx_est:
            st.markdown('<div class="warn-banner">⚠️ USD/KRW 환율 조회 실패 -- 추정값(₩1,480) 사용</div>',
                        unsafe_allow_html=True)

        regime_text = "🐂 강세장 (모멘텀 강화)" if is_bull else "🐻 약세장 (방어 모드)"
        c1, c2, c3 = st.columns(3)
        c1.markdown(f'<div class="metric-card"><div class="label">시장 국면</div><div class="value" style="font-size:1rem">{regime_text}</div></div>', unsafe_allow_html=True)
        c2.markdown(f'<div class="metric-card"><div class="label">환율 (USD/KRW)</div><div class="value">{fx:,.2f}</div></div>', unsafe_allow_html=True)
        c3.markdown(f'<div class="metric-card"><div class="label">포트폴리오 총액</div><div class="value">${total_usd:,.0f}</div></div>', unsafe_allow_html=True)
        st.write("")
        ...
        for t in tickers_r:
            ...
            rows.append({
                "티커": t,
                "목표 비중": f"{weights.get(t, 0):.1%}",
                "현재가 (USD)": f"${p:,.2f}",
                "매수금액 (KRW)": f"₩{buy_krw.get(t, 0):,.0f}",
                "매수금액 (USD)": f"${buy_usd.get(t, 0):,.2f}",
                "매수 수량": f"{buy_shares.get(t, 0):.4f}",
            })

        st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)

        total_buy = buy_krw.sum()
        st.markdown(f'<div class="success-banner">✅ 총 매수 금액: ₩{total_buy:,.0f}</div>',
                    unsafe_allow_html=True)

        # 비중 파이차트
        fig_pie = go.Figure(go.Pie(
            labels=tickers_r,
            values=[weights.get(t, 0) for t in tickers_r],
            hole=0.45,
...
with tab3:
    ...
    with col_bm:
        bm_input = st.text_input("벤치마크 티커 (쉼표 구분)",
    ...
    if run_bt:
        if not portfolio.tickers():
            ...
            def progress_cb(cur, total):
                ...
                progress_bar.progress(pct, text=f"백테스트 진행 중... ({cur}/{total} 주)")
    ...
    if "bt_result" in st.session_state:
        ...
        fig.add_trace(go.Scatter(
            x=df_bt.index, y=df_bt["Invested"],
            name="누적 투자금",
            fill="tozeroy",
        ...
        fig.add_trace(go.Scatter(
            x=df_bt.index, y=df_bt["KH_Strategy"],
            name="KH 전략",
        ...
        for col in all_cols:
            ...
            ret_pct = (final / invested - 1) * 100
            cagr = ((final / invested) ** (1 / max(years, 0.01)) - 1) * 100
            running_max = df_bt[col].cummax()
            mdd = ((running_max - df_bt[col]) / running_max).max() * 100
            summary_rows.append({
