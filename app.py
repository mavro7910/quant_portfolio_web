...
if not st.user.is_logged_in:
    ...
    <div class="main-header">
        ...
        <div>
            <h1>QPM <span>Alpha</span></h1>
            <p>Quantitative Portfolio Manager · Factor Momentum Strategy</p>
        </div>
    </div>
    """, unsafe_allow_html=True)
    st.markdown(
        '<div class="info-banner">🔐 <b>로그인이 필요합니다</b><br>'
        'Google 계정으로 로그인하면 어떤 기기에서든 동일한 포트폴리오를 사용할 수 있습니다.</div>',
        unsafe_allow_html=True,
    )
...
with col_user:
    ...
    st.markdown(
        ...
        f'<div class="user-chip" style="float:right">'
        f'<div class="avatar">{initials}</div>'
        f'<span>{_user_email}</span>'
        f'</div></div>',
        unsafe_allow_html=True,
    )
